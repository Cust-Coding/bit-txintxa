...
This project involves the development of a decentralized peer-to-peer (P2P) trading platform, built on the Nostr protocol and inspired by a social trust-based model similar to Vexl, with a strong focus on privacy, security, and user sovereignty.
The platform enables users to trade digital assets (primarily Bitcoin) directly with each other without relying on centralized intermediaries. Transactions are secured using a multi-signature escrow system, ensuring that funds are protected until both parties fulfill their obligations.
Communication, trade discovery, and order publishing are handled through the Nostr protocol, providing a decentralized, censorship-resistant infrastructure with no central servers.
The system incorporates a trust-based network model, allowing users to interact and trade based on social connections, reputation, and transaction history, similar to the approach used in the Vexl.

## Project Objectives
- Enable secure P2P trading without intermediaries
- Ensure user privacy and data ownership
- Promote a trust-based trading ecosystem
- Provide a censorship-resistant platform
- Deliver a simple and efficient trading experience

## Key Features (Contact-Based Model):
